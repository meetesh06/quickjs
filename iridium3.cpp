...
// Modified generateBytecode to include NOP removal
JSValue generateBytecode(JSContext *ctx, IridiumSEXP *node) {
    ...
    for (int i = 0; i < file->numArgs; ++i) {
        ...
        // Patch GOTOs first with original labels
        patchGotos(instructions);
        
        // Remove NOPs and adjust offsets
        vector<BCInstruction> optimizedInstructions = removeNOPs(ctx, instructions);
        
        // Generate the function with optimized instructions
        JSValue res = generateQjsFunction(ctx, bbContainer, optimizedInstructions);

        // Free BCLList
        freeBCLList(ctx, optimizedInstructions);
    ...
    // Fill CPool with closures (rest of the function remains the same)
    for (int i = 0; i < file->numArgs; i++) {
        IridiumSEXP *bbContainer = file->args[i];
        ensureTag(bbContainer, "BBContainer");
